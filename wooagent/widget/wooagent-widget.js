(function () {
  ...
  async function fetchGreeting() {
    ...
    if (!CFG.agent_api_url || CFG.agent_api_url === '') {
      addBubble('bot', '⚙️ WooAgent is not configured yet. Please go to WordPress Admin → WooAgent Settings and enter your backend URL.');
  ...
  async function fetchCart() {
    ...
    try {
      ...
      if (cartRes.ok && cartData && cartData.data) {
        cart = cartData.data;
      }
    } catch (e) {
      // ignore
  ...
  function isCheckoutPage() {
    return window.location.pathname.includes('checkout') || document.body.classList.contains('woocommerce-checkout');
  }
