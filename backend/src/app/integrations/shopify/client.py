...
class ShopifyClient(BaseStoreClient):
    ...
    # â”€â”€ Dynamic discount code generation (for promotional engine) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    async def generate_discount_code(
        ...
        """Generate a single-use discount code via Admin API discountCodeBasicCreate."""
        import uuid as _uuid
        code = f"SPEAKO-{_uuid.uuid4().hex[:8].upper()}"
        ...
        try:
            data = await self._admin_graphql(gql, variables)
            ...
            return {"success": True, "code": code, "discount_percent": discount_pct}
        except Exception as exc:
            logger.warning("generate_discount_code failed: %s", exc)
            return {"success": False, "code": code, "error": str(exc)}

    async def apply_discount_to_cart(self, *, session_id: str, coupon_code: str) -> Dict[str, Any]:
        """Apply a coupon code to the active Storefront cart via cartDiscountCodesUpdate."""
        cart_id = await self._get_cart_id(session_id)
        if not cart_id:
        ...
        try:
            data = await self._storefront(gql, {
                "cartId": cart_id,
                "discountCodes": [coupon_code],
            })
            result = data.get("cartDiscountCodesUpdate", {})
            errors = result.get("userErrors", [])
            if errors:
                return {"success": False, "message": str(errors)}
            cart_node = result.get("cart")
            if cart_node:
                return {"success": True, "cart": self._normalize_cart(cart_node), "code": coupon_code}
            return {"success": True, "code": coupon_code, "message": "Coupon applied."}
        except Exception as exc:
            logger.warning("apply_discount_to_cart failed: %s", exc)
