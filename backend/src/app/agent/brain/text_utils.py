...
def append_live_navigation(
    ...
    *,
    ...
    # Catalog / All-Products Navigation ("show all products", "browse catalog")
    if re.search(
        r"\b(show all (?:the )?products|all products|all items|entire catalog|full catalog|"
        r"browse (?:the )?(?:catalog|store|catalogue|everything)|view all products|open the catalog|catalog)\b",
        lower_query,
    ):
        _push(
            (base_url.rstrip("/") if base_url else "")
            + (("/collections/all") if platform != "woocommerce" else "/shop"),
            "catalog",
        )
        return

    # Collections / Categories Navigation ("show me all collections")
    if re.search(
        r"\b(all (?:the )?collections|show collections|browse collections|list collections|"
        r"collections|browse categories|shop by (?:category|collection)|categories)\b",
        lower_query,
    ):
        _push(
            (base_url.rstrip("/") if base_url else "")
            + (("/collections") if platform != "woocommerce" else "/product-category"),
            "collections",
        )
        return
    ...
    if search_match:
        ...
        if sq:
            ...
            _push(sq_url, "search", build_searchbox_spec(raw), filters)
        return
    ...
    if isinstance(products, list) and len(products) > 1:
        nav_query, filters = build_searchbar_query(query)
        spec = build_searchbox_spec(query)
    else:
        nav_query, filters, spec = query, None, query
    if nav_query:
        search_url = storefront_search_url(base_url, platform, nav_query)
        if search_url:
            _push(search_url, "search", spec, filters)
            return
...
def normalize_discovery_query(message: str) -> str:
    ...
    # 1) Size keywords FIRST — "size 9" must be removed as a unit. If the price
    #    strip ran first it would consume "9 dollars" and leave a bare "size"
    #    behind ("formal shoes size 9 under 100 dollars" → "formal shoes size").
    ...
    # Colour words + colour filler (structured `color` filter)
    cleaned = re.sub(
    ...
    # 2) Price/currency phrases BEFORE the stopword pass — "under 500" must be
    ...
    # 3) Stopword pass (filler/occasion words are not storefront search terms)
    cleaned = re.sub(
        r"\b(?:"
...
def build_searchbar_query(message: str) -> tuple[str, dict]:
    """Split a discovery utterance into (storefront query, structured filters).

    The storefront query carries the core terms AND the size/colour words (so a
    theme that ignores URL filter params still text-matches "size 9"), while
    price/occasion stay in the structured filters dict (price can never be a
    useful text-search term). Returns the storefront query + filters.
    """
    ...
    base = normalize_discovery_query(message)
    extra = []
    if nq.size:
        extra.append(f"size {nq.size}")
    if nq.color:
        extra.append(str(nq.color))
    sq = " ".join(p for p in [base] + extra if p).strip()
    return sq, filters


def build_searchbox_spec(message: str) -> str:
    """Human-readable spec for the search box ("formal shoes size 9 under 100").

    Mirrors exactly what the customer asked, so filling the theme's search input
    with this makes the result page feel aligned with the request.
    """
    from ..retrieval.normalizer import normalize

    nq = normalize(message or "")
    parts = [normalize_discovery_query(message)]
    if nq.size:
        parts.append(f"size {nq.size}")
    if nq.color:
        parts.append(str(nq.color))
    if nq.min_price is not None:
        parts.append(f"above {int(nq.min_price)}")
    if nq.max_price is not None:
        parts.append(f"under {int(nq.max_price)}")
