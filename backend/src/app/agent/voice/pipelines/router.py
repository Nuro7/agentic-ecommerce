...
Routing order (default):
  Pipeline A  →  OpenAI Realtime Provider (model selection preserved)
                 Circuit breaker: opens after 2 failures, resets after 30s

  Pipeline B  →  Gemini Live Provider (model selection preserved)
                 Circuit breaker: opens after 2 failures, resets after 30s
...
class PipelineRouter:
    ...
    Routes each voice WebSocket session through the selected provider or pipeline cascade:
      A (OpenAI Realtime) → B (Gemini Live) → C (Text fallback)
    ...
    async def run(self, websocket: Any, session_id: str, store_client: Any = None,
        ...
        # ── 2. Explicit Gemini Live Provider ──
        if provider_choice == "gemini_live":
            logger.info("Using Gemini Live provider: session=%s", session_id)
            try:
                ...
                logger.error("Gemini Live provider failed session=%s: %s", session_id, exc, exc_info=True)
                await self._notify_fallback(
                    websocket, from_pipeline="GeminiLive", to_pipeline="C",
                    message="Gemini Live voice service interrupted. Switching to text mode.",
                )
                await self._run_text_fallback(websocket, session_id, store_client, tenant_id)
                return

        # ── 3. Default Cascading: OpenAI Realtime (A) -> Gemini Live (B) -> Text (C) ──
        
        # ── Pipeline A (OpenAI Realtime) ──
        if self._breaker_a.is_available():
            try:
                ...
                logger.error(
                    "Pipeline A (OpenAI Realtime) failed session=%s: %s: %s | breaker=%s",
        ...
        if self._breaker_b.is_available():
            try:
                ...
                logger.error(
                    "Pipeline B (Gemini Live) failed session=%s: %s: %s | breaker=%s",
        ...
        return {
            ...
            "pipeline_a": {
                **self._breaker_a.health(),
                "description": "OpenAI Realtime Voice via Turn Coordinator",
            },
            "pipeline_b": {
