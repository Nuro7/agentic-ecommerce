...
from services.base_store_client import BaseStoreClient
...
class ShopifyClient(BaseStoreClient):
    ...
    async def _admin_get(self, path: str, params: Optional[Dict] = None) -> Any:
        """GET request to Shopify Admin REST API."""
    ...
    async def _set_cart_id(self, session_id: str, cart_id: str) -> None:
        if not self.redis:
            return
        try:
            # Cart IDs live as long as a session (7 days)
    ...
    def _normalize_product_node(self, node: Dict[str, Any]) -> Dict[str, Any]:
        """
        Convert a Shopify Storefront product node into the normalized shape
        that matches WooCommerceClient output so the agent tools stay unchanged.
        """
        ...
        # Build variations_summary from variants
        variations_summary = []
        ...
        # Attributes: collect all option names/values
        attributes = []
    ...
    async def search_products(
        ...
        *,
        ...
        # Build Shopify product filter query string
    ...
    async def get_product_details(self, product_id: int) -> Dict[str, Any]:
        ...
        try:
            data = await self._storefront(
                GQL, {"id": _int_to_product_gid(product_id)}
            )
    ...
    async def check_inventory(
        ...
        *,
        ...
        if attributes and variations:
            ...
            # Attributes specified but no match found
            return {
                "product_id": product_id,
                "variation_id": 0,
                "in_stock": False,
    ...
    def _normalize_cart(self, cart_node: Dict[str, Any]) -> Dict[str, Any]:
        """Convert a Shopify cart GQL node into the normalized cart shape."""
    ...
    async def _create_cart(self, session_id: str) -> str:
        """Create a new Shopify cart and persist its ID in Redis."""
        GQL = """
        mutation CartCreate {
          cartCreate {
            cart { id checkoutUrl }
    ...
    async def add_to_cart(
        ...
        *,
        ...
        try:
            ...
            # Resolve variant GID: if variation_id given use it, else pick first in-stock variant
            if variation_id and variation_id > 0:
                ...
                if chosen is None:
                    # Simple product — use first variant from Storefront directly
                    GQL = """
    ...
    async def remove_from_cart(
        ...
        *,
        ...
        try:
            ...
            """
            data = await self._storefront(
                GQL, {"cartId": cart_id, "lineIds": [line_id]}
            )
    ...
    async def apply_coupon(self, *, session_id: str, coupon_code: str) -> Dict[str, Any]:
        """
        Apply a discount code to the Shopify cart via cartDiscountCodesUpdate.
        """
        ...
        try:
            ...
            """
            data = await self._storefront(
                GQL, {"cartId": cart_id, "discountCodes": [code]}
            )
    ...
    async def get_best_coupon(self, cart_total: float = 0) -> Dict[str, Any]:
        """
        Fetch active price rules from Shopify Admin and return the best one.
        Falls back gracefully if Admin token is not configured.
        """
        try:
            ...
            for rule in price_rules:
                ...
                min_amt = _safe_float(rule.get("prerequisite_subtotal_range", {}).get("greater_than_or_equal_to", 0) if rule.get("prerequisite_subtotal_range") else 0)
                ...
                # Fetch one discount code for this rule
                try:
        ...
        *,
        ...
        try:
            ...
            for order in orders:
                ...
                tracking_url = None
                if fulfillment:
                    tracking_url = fulfillment[0].get("tracking_url")
    ...
    async def get_reviews(self, product_id: int) -> Dict[str, Any]:
        """
        Shopify has no native review API in Storefront/Admin.
        Returns empty structure — reviews are typically via apps (Judge.me, Yotpo).
        If SHOPIFY_REVIEWS_ENDPOINT is set, proxies to that URL instead.
        """
    ...
    async def submit_review(
        ...
        *,
        ...
        """Shopify has no native review submission endpoint — placeholder."""
        return {
            "success": False,
    ...
    async def get_store_policies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            shipping_body = _strip_html(
                (shop.get("shippingPolicy") or {}).get("body", "")
            )[:300]
            refund_body = _strip_html(
                (shop.get("refundPolicy") or {}).get("body", "")
            )[:300]
    ...
    def _attributes_to_variation_map(_attributes: Any) -> Dict[str, str]:
        """
        Shopify uses variant GIDs resolved in add_to_cart — no WC attribute map needed.
        Returns empty dict; the orchestrator passes it as `variation=` to add_to_cart
        which ShopifyClient ignores (it uses variation_id instead).
        """
        return {}

    async def get_live_cart(self, session_id: str) -> Dict[str, Any]:
        """Alias used by the orchestrator — delegates to get_cart."""
        return await self.get_cart(session_id=session_id)

    # ── Cache warm-up ──────────────────────────────────────────────────────────

    async def pre_warm(self) -> None:
        """Pre-fetch categories and top products into Redis cache."""
        try:
