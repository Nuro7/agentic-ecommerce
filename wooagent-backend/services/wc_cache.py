...
class CachedWooCommerceClient:
    ...
    async def search_products(self, query: str = "", **filters) -> list:
        ...
        # Only use cache if it has results — empty lists may be stale from past failures
    ...
    async def get_cart(self, *args, **kwargs):
        return await self.wc.get_cart(*args, **kwargs)   # cart is live — never cache
