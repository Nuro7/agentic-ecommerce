...
class WooCommerceClient:
    ...
    def __init__(
        ...
        # Stable 8-char prefix derived from the store URL so multiple stores
        # sharing the same Redis instance never collide on cache keys.
    ...
    async def search_products(
        ...
        *,
        ...
        # 1) Prefer plugin endpoint (most reliable in shared-host/local setups)
        try:
        ...
        if not rows and self.enable_wc_v3_fallback and self.consumer_key and self.consumer_secret:
            ...
            try:
                ...
                # Graceful fallback: many local/shared hosts block wc/v3 key auth.
                logger.warning("wc/v3 product fallback unavailable: %s", exc)
                rows = []

        # 4) Last fallback for generic intent-like queries:
        # If query is effectively "show products/list items", fetch a catalog slice.
        ...
        # Only cache non-empty results — empty results may be transient failures
        # and should not poison the cache for up to 5 minutes.
        if products:
    ...
    async def check_inventory(
        ...
        *,
        ...
        # If specific attributes are requested for a variable product and no variant matches,
        # treat that request as unavailable instead of falling back to parent stock.
    ...
    async def add_to_cart(
        ...
        *,
        ...
        try:
            ...
            # If variation missing for variable product, auto-pick first in-stock variation.
    ...
    async def apply_coupon(self, *, session_id: str, coupon_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fall back: store code in session so it can be applied at checkout
            return {
                "success": True,
                "code": code,
    ...
    async def get_categories(self) -> List[Dict[str, Any]]:
        ...
        rows: List[Dict[str, Any]] = []
        # Try plugin endpoint first (no auth issues)
        try:
    ...
    async def get_store_info(self) -> Dict[str, Any]:
        ...
        # Pull live settings from WooCommerce REST API in parallel
        ...
        # Remove empty string fields so LLM doesn't see blank values
    ...
    async def _request(
        ...
        *,
        ...
        if signed:
            ...
            # Include path in payload — must match class-wooagent-auth.php sign_payload()
            # and services/security.py compute_signature().
            signature = hmac.new(
    ...
    def _normalize_product_rows(
        ...
        *,
        ...
        for row in rows if isinstance(rows, list) else []:
            ...
            image_url = ""
            # 1) PHP plugin endpoint returns 'image_url' directly as a string
            direct_img = row.get("image_url")
            if isinstance(direct_img, str) and direct_img.startswith("http"):
                image_url = direct_img
            # 2) Store/WC API returns 'images' as a list of dicts
            if not image_url:
                ...
                if isinstance(images_field, list) and images_field:
                    ...
                    if isinstance(first, dict):
                        ...
                        image_url = first
            # 3) Store API singular 'image' field (dict with 'src')
            if not image_url:
    ...
    def _normalize_product_detail(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for image in data.get("images", []) if isinstance(data.get("images"), list) else []:
            # PHP plugin returns images as plain URL strings; Store API returns dicts
        ...
        # Also try singular 'image' field (Store API format)
        if not images:
    ...
    def _is_in_stock(record: Any) -> bool:
        ...
        if stock_status:
            # onbackorder = purchasable in WooCommerce (customers can still order)
        ...
        # No stock info at all — assume in stock (WooCommerce default for unmanaged products)
        return True
    ...
    async def get_live_cart(self, session_id: str) -> dict:
        """
        Fetch LIVE cart from WooCommerce — always fresh.
        This is the source of truth. Redis is just a cache.
        """
        try:
            # Call the custom plugin endpoint
            result = await self._plugin_get(
                '/cart',
                params={'session_id': session_id}
            )
            
            cart = result.get('cart', {})
            # If the result itself is the cart and doesn't wrap in 'cart', adjust
            if 'contents' not in cart and 'items' in result:
                cart = result
                
            items = []
            
            for key, item in (cart.get('contents', {}) or {}).items():
                product_id   = item.get('product_id', 0)
                variation_id = item.get('variation_id', 0)
                quantity     = int(item.get('quantity', 1) or 1)
                data         = item.get('data', {}) or {}

                unit_price = self._safe_float(data.get('price', 0))
                ...
                items.append({
                    ...
                    'name':          data.get('name', ''),
                    'quantity':      quantity,
                    'price':         f"{sym}{unit_price:,.0f}",
                    'line_total':    quantity * unit_price,
                    'image_url':     '',
                })

            total      = self._safe_float(cart.get('total', 0))
            ...
            return {
                ...
                'subtotal':   f"{sym}{total:,.0f}",
                'total':      f"{sym}{total:,.0f}",
                'coupon_codes': cart.get('applied_coupons', [])
            }

        except Exception as e:
            logger.error(f"Live cart fetch failed: {e}")
            sym = self.currency_symbol
            return {
                'success': False, 'is_empty': True,
                'items': [], 'item_count': 0,
                'total': f'{sym}0', 'subtotal': f'{sym}0'
            }
    ...
    async def update_cart_quantity(
        ...
        *,
        ...
        try:
            ...
            cart = await self.get_live_cart(session_id)
            return {
                'success':     True,
    ...
    async def get_product_variations(self, product_id: int) -> dict:
        """
        Get ALL variations for a variable product with stock per variant.
        Critical for size/color selection flow.
        """
        try:
    ...
    async def get_best_coupon(
        ...
        """
        Find the best available coupon for current cart total.
        Fetches all active non-expired coupons and picks most valuable.
        """
        try:
                ...
                if c.get('date_expires'):
                    try:
                        exp = datetime.fromisoformat(
                            c['date_expires'].replace('Z', '+00:00')
                        )
                        if exp < now:
                            continue
                    except Exception:
    ...
    async def submit_review(
        ...
        *,
        ...
        """Submit product review via WooCommerce API."""
        try:
    ...
    async def get_reviews(self, product_id: int) -> dict:
        """Fetch product reviews from WooCommerce (plugin endpoint first, WC API fallback)."""
        try:
            # Try plugin endpoint first (no auth issues)
            try:
    ...
    async def get_store_policies(self) -> dict:
        """
        Fetch store policies: shipping, returns, payment.
        Cached for 1 hour — policies rarely change.
        """
