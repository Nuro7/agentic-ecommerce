...
async def execute_tool(
    ...
    if tool_name == "search_products":
        products = await woocommerce_service.search_products(
    ...
    if tool_name == "get_product_details":
        ...
        product = await woocommerce_service.get_product_details(product_id)
    ...
    if tool_name == "check_inventory":
        inventory = await woocommerce_service.check_inventory(
            product_id=_to_int(args.get("product_id"), default=0),
    ...
    if tool_name == "get_cart":
        cart = await woocommerce_service.get_cart(session_id=session_id)
        return ToolExecution(
    ...
    if tool_name == "add_to_cart":
        ...
        if not variation_id and args.get("attributes"):
            inv = await woocommerce_service.check_inventory(
        ...
        cart_result = await woocommerce_service.add_to_cart(
    ...
    if tool_name == "remove_from_cart":
        remove_result = await woocommerce_service.remove_from_cart(
            session_id=session_id,
    ...
    if tool_name == "get_orders":
        orders = await woocommerce_service.get_orders(
    ...
    if tool_name == "apply_coupon":
        coupon = await woocommerce_service.apply_coupon(
            session_id=session_id,
    ...
    if tool_name == "get_store_info":
        info = await woocommerce_service.get_store_policies()
        return ToolExecution(
    ...
    if tool_name == "compare_products":
        ...
        for query in [product_a, product_b]:
            if not query:
                continue
            rows = await woocommerce_service.search_products(
                query=query,
                in_stock_only=False,
                limit=1,
            )
    ...
    if tool_name == "update_cart_quantity":
        ...
        result = await woocommerce_service.update_cart_quantity(
            session_id=session_id, product_id=pid, new_quantity=qty
        )
        return ToolExecution(
    ...
    if tool_name == "find_variants":
        pid = _to_int(args.get("product_id"), default=0)
        result = await woocommerce_service.get_product_variations(product_id=pid)
        return ToolExecution(
    ...
    if tool_name == "get_best_coupon":
        ...
        result = await woocommerce_service.get_best_coupon(cart_total=total)
        return ToolExecution(
    ...
    if tool_name == "submit_review":
        ...
        result = await woocommerce_service.submit_review(
            product_id=pid, rating=rating, review_text=text, reviewer_name=name
        )
        return ToolExecution(
